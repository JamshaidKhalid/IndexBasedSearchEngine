import json
from nltk.corpus.reader import nps_chat
...
def scoreGenerator(hitlists):
  # Function for generating score having the args of indexes
    ...
    # for hitlist in hitlists:
        # hitlist_lens.append(len(hitlist))
        
    # now fetching the length of one word like chill

    hitlist_is = [0] * n
    
    # initalizing the 2D array with the 0
      ...
      try:
        ...
        for word in search:
            if (word not in stop_words):
                search_word_ids.append(lexFileData[word])
        # Having the word Id's from lexicon of the word entered by user
        for word_id in search_word_ids:
            if word_id != -1:
                inverted_index_entries.append(invertedFileData[str(word_id)])
