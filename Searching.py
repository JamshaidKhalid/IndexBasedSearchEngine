...
def singleWordSearch(searchInput):
    try:
        ...
        docs = OrderedDict(sorted(numberOfDocuments.items(),
        ...
        print(f"\"{searchInput}\" word doesn't exist in our data")
...
def scoreGenerator(hitlists):
    ...
    while hitlist_is != hitlist_lens:
        ...
        for i in range(n):
            if hitlist_is[i] == len(hitlists[i]):
                continue
...
if(len(search) > 1):
    try:
        ...
        # search_word_ids = []
        search_word_ids = [lexFileData[word]
                           for word in search if ((word not in stop_words))]
        # Having the word Id's from lexicon of the word entered by user
        inverted_index_entries = [invertedFileData[str(
            word_id)] for word_id in search_word_ids if word_id != -1]
        ...
        for i, iie in enumerate(inverted_index_entries):
            ...
            for doc in iie:
                ...
                docs_with_score.append(
                    (doc, scoreGenerator(current_doc_hitlists)))
